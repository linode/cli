...
sub list {
    ...
    return  $args{output_format} eq 'raw' ? $out_hashref :
...
sub show {
    ...
    for my $object_label (keys %{$self->{object}}) {
        next if ($label && $object_label ne $label);
        for my $key (keys %{$self->{object}->{$object_label}}) {
            $return .= sprintf("\n%24s %-32s", $key, $self->{object}->{$object_label}->{$key});
        }
...
sub _poll_and_wait {
    ...
    my $poll_result = try {
        for (my $i = 0; $i <= 60; $i++) {
            my $job_success = $api_obj->linode_job_list(
                linodeid    => $linode_id,
                jobid       => $job_id,
            )->[0]->{host_success};

            if ($job_success || ($i == 60 && !$job_success)) {
                print STDOUT "\n" if ($output_format eq 'human');
            }

            return 1 if $job_success;
            return 0 if ($i == 60 && !$job_success);
